...
-- ============================================================
-- INFINITE JUMP BACKEND (integrated)
-- ============================================================
...
-- STATE
-- ============================================================
local ConfigFileName = "FIVE_HUB_Config.json"
local Enabled = {
    SpeedBoost = false,
    AntiRagdoll = false,
    SpinBot = false,
    ...
    ScriptUserESP = true,
    InfiniteJump = true,    -- Linked to infinityJumpEnabled
    NoDie = false,
    Aimbot = false,
    Float = false,
    FullAutoDuel = false,
...
local Values = {
    ...
    FOV = 70,
    FloatHeight = 8,
    JumpForce = 50,          -- For infinite jump
    ClampFallSpeed = 80,     -- For infinite jump
}
local KEYBINDS = {
    ...
    INFJUMP = Enum.KeyCode.X,   -- Keybind for toggling infinite jump
}
...
-- sync infinite jump with Enabled table
infinityJumpEnabled = Enabled.InfiniteJump
jumpForce = Values.JumpForce
clampFallSpeed = Values.ClampFallSpeed

-- ============================================================
-- BACKEND (existing code continues...)
-- ============================================================
local Connections = {
    infiniteJump = {heartbeat=nil,jumpRequest=nil}
}
...
local function createSpamBatCircle()
    ...
    circle.Name="FIVE_SpamBatCircle"; circle.Anchored=true; circle.CanCollide=false
...
local function setupFloatObjects(char)
    ...
    floatAttachment.Name = "FIVE_FloatAttachment"
    floatAttachment.Parent = hrp
    floatVectorForce = Instance.new("VectorForce")
    floatVectorForce.Name = "FIVE_FloatForce"
...
local coordESPFolder = Instance.new("Folder",workspace); coordESPFolder.Name="FIVE_CoordESP"
    ...
    fadMoveConn = RunService.Stepped:Connect(function()
        ...
        if dist < 5 then
            if (fadCurrent == 4 or fadCurrent == 6) and not fadGrabDone then
                ...
                if c2 then
                    local h=c2:FindFirstChildOfClass("Humanoid")
...
local function _abStart()
    _abStop()
    local char=Player.Character
    if char then _abApplyNoDie(char) end
        ...
        pcall(function()
            ...
            for _,p in ipairs(Players:GetPlayers()) do
                if p~=Player and p.Character then
                    local oh=p.Character:FindFirstChild("HumanoidRootPart")
                    if oh then
                        local d=(myHRP.Position-oh.Position).Magnitude
            ...
            local tHRP=_abTarget.Character:FindFirstChild("HumanoidRootPart")
...
Player.CharacterAdded:Connect(function(nc) if not abActive then return end task.wait(0.5); _abApplyNoDie(nc); _abStart() end)
...
local function createSpeedLabel(char)
    ...
    bb.Name = "FIVE_SpeedLabel"; bb.AlwaysOnTop = false
...
local function onTap(btn, cb) if isMobile then btn.Activated:Connect(cb) else btn.MouseButton1Click:Connect(cb) end end
local C = {
...
sg.Name="FIVE_CLEAN"; sg.ResetOnSpawn=false
...
titleTxt.BackgroundTransparency=1; titleTxt.Text="FIVE HUB"; titleTxt.TextColor3=C.white
...
closeBtn.BackgroundColor3=C.dangerDark; closeBtn.Text="Ã—"; closeBtn.TextColor3=C.white
...
local function createKeybindBadge(parent, keybindKey)
    ...
    btn.Text = (kc0 == Enum.KeyCode.Unknown) and "â€”" or kc0.Name
    ...
    btn.MouseButton1Click:Connect(function()
        if waitingForKeybind == keybindKey then
            ...
            btn.Text = (kc2 == Enum.KeyCode.Unknown) and "â€”" or kc2.Name
        ...
        if waitingForKeybind then
            ...
            if prev then
                ...
                prev.Text = (pkc == Enum.KeyCode.Unknown) and "â€”" or pkc.Name
...
local function createToggle(order, labelTxt, enabledKey, callback, _unused, keybindKey)
    ...
    local function doToggle()
        ...
        if enabledKey == "InfiniteJump" then
            infinityJumpEnabled = isOn
        end
    end
    if isMobile then
...
-- ============================================================
local o = 0
createSectionHeader(o,"MOVEMENT")
o+=1
createToggle(o,"Speed Boost","SpeedBoost", function(s) Enabled.SpeedBoost=s; if s then startSpeedBoost() else stopSpeedBoost() end end, nil,"SPEED")
o+=1
createSlider(o,"Boost Speed",1,70,"BoostSpeed",function(v) Values.BoostSpeed=v end)
o+=1
createToggle(o,"Speed While Stealing","SpeedWhileStealing", function(s) Enabled.SpeedWhileStealing=s; if s then startSpeedWhileStealing() else stopSpeedWhileStealing() end end, nil, nil)
o+=1
createSlider(o,"Carry Speed",10,35,"StealingSpeedValue",function(v) Values.StealingSpeedValue=v end)
o+=1
createToggle(o,"Boosters (Galaxy)","Galaxy", function(s) Enabled.Galaxy=s; if s then startGalaxy() else stopGalaxy() end end, nil,"GALAXY")
o+=1
createSlider(o,"Gravity %",25,130,"GalaxyGravityPercent", function(v) Values.GalaxyGravityPercent=v; if galaxyEnabled then adjustGalaxyJump() end end)
o+=1
createSlider(o,"Jump Boost",10,80,"HOP_POWER",function(v) Values.HOP_POWER=v end)
o+=1
createToggle(o,"Anti Ragdoll","AntiRagdoll", function(s) Enabled.AntiRagdoll=s; if s then startAntiRagdoll() else stopAntiRagdoll() end end, nil,"ANTIRAGDOLL")
o+=1
createToggle(o,"Spin Bot","SpinBot", function(s) Enabled.SpinBot=s; if s then startSpinBot() else stopSpinBot() end end, nil,"SPIN")
o+=1
createSlider(o,"Spin Speed",5,50,"SpinSpeed",function(v) Values.SpinSpeed=v end)
o+=1
createToggle(o,"Float","Float", function(s) Enabled.Float=s; if s then startFloat() else stopFloat() end end, nil,"FLOAT")
o+=1
createSlider(o,"Float Height",1,50,"FloatHeight",function(v) Values.FloatHeight=v end)
o+=1
createSectionHeader(o,"AUTO WALK")
o+=1
createToggle(o,"Auto Walk Left","AutoWalkEnabled", function(s) AutoWalkEnabled=s; Enabled.AutoWalkEnabled=s; if s then startAutoWalk() else stopAutoWalk() end end, nil,"AUTOLEFT")
o+=1
createToggle(o,"Auto Walk Right","AutoRightEnabled", function(s) AutoRightEnabled=s; Enabled.AutoRightEnabled=s; if s then startAutoRight() else stopAutoRight() end end, nil,"AUTORIGHT")
o+=1
createToggle(o,"Full Auto Play","FullAutoDuel", function(s) Enabled.FullAutoDuel=s if s then startFullAutoDuel() else stopFullAutoDuel() end end, nil,"FULLAUTODUEL")
o+=1
createSectionHeader(o,"STEAL")
o+=1
createToggle(o,"Auto Grab","AutoSteal", function(s) Enabled.AutoSteal=s; if s then startAutoSteal() else stopAutoSteal() end end, nil, nil)
o+=1
createSectionHeader(o,"MISC")
o+=1
createToggle(o,"Infinite Jump","InfiniteJump", function(s)
    Enabled.InfiniteJump=s
    infinityJumpEnabled=s
    if s then
        -- params already set via Values
    else
        -- disable (jump override stops)
    end
end, nil, "INFJUMP")
o+=1
createSlider(o,"Jump Force",20,150,"JumpForce",function(v) Values.JumpForce=v; jumpForce=v end)
o+=1
createSlider(o,"Fall Clamp",30,200,"ClampFallSpeed",function(v) Values.ClampFallSpeed=v; clampFallSpeed=v end)
o+=1
createToggle(o,"Optimizer + XRay","Optimizer", function(s) Enabled.Optimizer=s; if s then enableOptimizer() else disableOptimizer() end end, nil, nil)
o+=1
createToggle(o,"Unwalk","Unwalk", function(s) Enabled.Unwalk=s; if s then startUnwalk() else stopUnwalk() end end, nil, nil)
o+=1
createToggle(o,"Spam Bat","SpamBat", function(s) Enabled.SpamBat=s; if s then startSpamBat();startSpamBatCircle() else stopSpamBat();removeSpamBatCircle() end end, nil, nil)
o+=1
createSlider(o,"FOV",30,120,"FOV",function(v) Values.FOV=v pcall(function() local cam=workspace.CurrentCamera; if cam then cam.FieldOfView=v end end) end)
o+=1
if not isMobile then
    createToggle(o,"Aimbot","Aimbot", function(s) if s==abActive then return end; abToggle() end, nil,"AIMBOT")
    o+=1
    abVisualSetter = function(on) if VisualSetters.Aimbot then VisualSetters.Aimbot(on,true) end end
end
createSectionHeader(o,"CONFIG")
o+=1
createButtonRow(o,"ðŸ’¾ SAVE CONFIG",Color3.fromRGB(28,28,28),C.dim, function(btn)
    local ok = SaveConfig()
    btn.Text=ok and "âœ“ SAVED!" or "âœ— FAILED"
    btn.BackgroundColor3=ok and C.off or C.dangerDark
    task.delay(2,function() btn.Text="ðŸ’¾ SAVE CONFIG"; btn.BackgroundColor3=Color3.fromRGB(28,28,28) end)
    playSound("rbxassetid://6895079813",0.4,ok and 1.2 or 0.7)
end)
o+=1
createButtonRow(o,"ðŸ—‘ RESET CONFIG",Color3.fromRGB(55,10,10),C.dangerDark, function(btn)
    ...
    KEYBINDS.INFJUMP=Enum.KeyCode.X
    ...
    for k,badgeBtn in pairs(KeyBindBtns) do local kc=KEYBINDS[k] badgeBtn.Text = (kc==Enum.KeyCode.Unknown) and "â€”" or kc.Name end
    ...
    btn.Text="âœ“ RESET!"; btn.BackgroundColor3=C.off
    task.delay(1.8,function() btn.Text="ðŸ—‘ RESET CONFIG"; btn.BackgroundColor3=Color3.fromRGB(55,10,10) end)
    playSound("rbxassetid://6895079813",0.5,0.8)
end)
o+=1

-- ============================================================
-- MINI BUTTON (menu icon)
-- ============================================================
...
miniBtn.BackgroundColor3=C.bg; miniBtn.Text="5"; miniBtn.TextColor3=C.white
miniBtn.Font=Enum.Font.GothamBlack; miniBtn.TextSize=math.floor(18*GS)
miniBtn.BorderSizePixel=0; miniBtn.Visible=false
rc(miniBtn, math.floor(10*GS)); st(miniBtn, 3, C.white, 0)
onTap(miniBtn, function() main.Visible=true; miniBtn.Visible=false; playSound("rbxassetid://6895079813",0.4,1) end)
onTap(closeBtn, function() main.Visible=false; miniBtn.Visible=true; playSound("rbxassetid://6895079813",0.4,1) end)
...
if isMobile then
    ...
    mobSG.Name="FIVE_MOB"; mobSG.ResetOnSpawn=false; mobSG.DisplayOrder=999; mobSG.Parent=Player.PlayerGui
    ...
    local spacing = 58
    local alSetOn, alSetCb = makeDraggableMobBtn(mobSG, Vector2.new(startX, startY + spacing*0), "AUTO LEFT")
    local alActive = false
    alSetCb(function()
        alActive = not alActive; alSetOn(alActive)
        if alActive then
            if AutoRightEnabled then AutoRightEnabled=false; Enabled.AutoRightEnabled=false; stopAutoRight()
                if VisualSetters.AutoRightEnabled then VisualSetters.AutoRightEnabled(false,true) end end
            AutoWalkEnabled=true; Enabled.AutoWalkEnabled=true
            if VisualSetters.AutoWalkEnabled then VisualSetters.AutoWalkEnabled(true,true) end
            startAutoWalk()
        else
            AutoWalkEnabled=false; Enabled.AutoWalkEnabled=false; stopAutoWalk()
            if VisualSetters.AutoWalkEnabled then VisualSetters.AutoWalkEnabled(false,true) end
        end
        playSound("rbxassetid://6895079813", 0.3, alActive and 1.1 or 0.9)
    end)
    local arSetOn, arSetCb = makeDraggableMobBtn(mobSG, Vector2.new(startX, startY + spacing*1), "AUTO RIGHT")
    local arActive = false
    arSetCb(function()
        arActive = not arActive; arSetOn(arActive)
        if arActive then
            if AutoWalkEnabled then AutoWalkEnabled=false; Enabled.AutoWalkEnabled=false; stopAutoWalk()
                if VisualSetters.AutoWalkEnabled then VisualSetters.AutoWalkEnabled(false,true) end end
            ...
            AutoRightEnabled=false; Enabled.AutoRightEnabled=false; stopAutoRight()
            if VisualSetters.AutoRightEnabled then VisualSetters.AutoRightEnabled(false,true) end
        end
        playSound("rbxassetid://6895079813", 0.3, arActive and 1.1 or 0.9)
    end)
    local abSetOn, abSetCb = makeDraggableMobBtn(mobSG, Vector2.new(startX, startY + spacing*2), "AIMBOT")
    abVisualSetter = function(on) abSetOn(on) end
    abSetCb(function() abToggle() playSound("rbxassetid://6895079813", 0.3, abActive and 1.1 or 0.9) end)
    local sbSetOn, sbSetCb = makeDraggableMobBtn(mobSG, Vector2.new(startX, startY + spacing*3), "STEAL BOOST")
    ...
    local ijSetOn, ijSetCb = makeDraggableMobBtn(mobSG, Vector2.new(startX, startY + spacing*5), "INF JUMP")
    local ijActive = true
    ijSetCb(function()
        ijActive = not ijActive; ijSetOn(ijActive)
        Enabled.InfiniteJump = ijActive
        infinityJumpEnabled = ijActive
        if VisualSetters.InfiniteJump then VisualSetters.InfiniteJump(ijActive, true) end
        playSound("rbxassetid://6895079813", 0.3, ijActive and 1.1 or 0.9)
    end)
    VisualSetters.InfiniteJump = function(on, skipCB)
        ijActive = on
        ijSetOn(on)
...
task.spawn(function()
    ...
    for k,badgeBtn in pairs(KeyBindBtns) do
        local kc=KEYBINDS[k]
        badgeBtn.Text = (kc==Enum.KeyCode.Unknown) and "â€”" or kc.Name
    end
...
local function dispatchKey(kc)
    ...
    if kc==KEYBINDS.AIMBOT and KEYBINDS.AIMBOT~=Enum.KeyCode.Unknown then
        abToggle()
    end
    ...
    if kc==KEYBINDS.INFJUMP and KEYBINDS.INFJUMP~=Enum.KeyCode.Unknown then
...
print("FIVE HUB Loaded ” Press U to toggle GUI. Infinite Jump integrated.")
