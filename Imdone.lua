...
--// ===== WAYPOINTS (NINE LOGIC) =====
local rightWaypoints = {
    Vector3.new(-473, -7, 30),
    Vector3.new(-483, -5, 18),
    Vector3.new(-475, -7, 26),
    Vector3.new(-474, -7, 105),
}

local leftWaypoints = {
    Vector3.new(-472, -7, 90),
    Vector3.new(-484, -5, 100),
    Vector3.new(-475, -7, 93),
    Vector3.new(-474, -7, 16),
}

--// ===== STATE (KAWATAN STYLE TOGGLE) =====
local patrolMode = "none"
local currentWaypoint = 1
local waitingLeft = false
local waitingRight = false

local function startMove(mode)
    patrolMode = mode
    currentWaypoint = 1
end
...
local function getWaypoints()
    return patrolMode == "left" and leftWaypoints or rightWaypoints
end

--// ===== UI =====
pcall(function()
    CoreGui:FindFirstChild("AutoUI"):Destroy()
end)
...
local leftBtn = makeBtn("Auto Left", 10)
local rightBtn = makeBtn("Auto Right", 130)

-- TOGGLE (KAWATAN STYLE)
leftBtn.MouseButton1Click:Connect(function()
    if waitingLeft or patrolMode == "left" then
        stopMove()
        leftBtn.Text = "Auto Left"
    else
        waitingLeft = true
        waitingRight = false
...
rightBtn.MouseButton1Click:Connect(function()
    if waitingRight or patrolMode == "right" then
        stopMove()
        rightBtn.Text = "Auto Right"
    else
        waitingRight = true
        waitingLeft = false
